...
function nmdrTable(id) {
    ...
    $.makeView = function () {
		...
		style = "<style type='text/css'>" +
			...
			" padding-top: " + (pr.headerPadding + 1) + "px;" + (pr.columnsResizable ? " padding-right: 0px;" : "") +
            " text-align: left;" +
            ...
            pfx + " .nmdrTB_header_th_rs { width:4px; cursor: col-resize; }" +
			...
			" padding: " + pr.bodyPadding + "px;" + (pr.columnsResizable ? " padding-right: 0px;" : "") +
			...
			" transition: all 0.5s ease-in-out;" +
            "}" +
			".nmdrTBFcheckbox:hover {cursor:pointer; background:#0073C6; color:#FFFFFF;}" +
            pfx + " .nmdrTB_rowMenu {" +
			" display: none;" +
			" background-color: white;" +
			" border: 1px solid #bbb;" +
    ...
    $.makeHeader = function () {
        ...
        for (var col = 0; col < this.columns.length; col++) {
			...
			var cw = this.getColumnWidth(col),
				...
				rs = this.props.columnsResizable ? ("<td class='nmdrTB_header_th_rs' " +
					"onmousedown=\"nmdr.core.$('" + this.id + "').resizeColumn(event, 0," + col + ");\" " + 
					"onmouseover=\"nmdr.core.$('" + this.id + "').resizeColumn(event, 1," + col + ");\" " + 
					"onmouseout=\"nmdr.core.$('" + this.id + "').resizeColumn(event, 2," + col + ");\" " + 
					"onmousemove=\"nmdr.core.$('" + this.id + "').resizeColumn(event, 3," + col + ");\" " +
					"onmouseup=\"nmdr.core.$('" + this.id + "').resizeColumn(event, 4," + col + ");\"></td>") : "";
			...
			buf.push("<td style='padding:0px;'><div class='nmdrTB_header_th_dv' onClick=\"nmdr.core.$('" + this.id + "').sortColumn(" + col + ");\">" + (ct ? ct : this.columns[col]) + "</div></td>");
			buf.push("<td style='width:52px; padding:0px;'><table border='0' cellpadding='0' cellspacing='0'><tr>");
			buf.push("<td style='width:16px; padding:0px 0px 1px 0px;' onClick=\"nmdr.core.$('" + this.id + "').sortColumn(" + col + ");\">" + s1 + "</td>");
			buf.push("<td style='width:16px; padding:0px 0px 1px 0px;' onClick=\"nmdr.core.$('" + this.id + "').sortColumn(" + col + ");\">" + s2 + "</td>");
			buf.push("<td style='width:16px; padding:0px; text-align:right;'>");
    ...
    $.makeBody = function () {
        ...
        if (this.dataRows.length > 0) {
            ...
            for (var row = first; row <= last; row++) {
                ...
                var data = this.dataRows[row - 1].split(this.props.dataSplitter),
                ...
                for (var col = 0; col < data.length; col++) {
					...
					bc = this.renderBodyCell(row - 1, col, data[col], se),
					ac = this.props.showRowCommands && col == data.length - 1 ?
					"<table width='100%' height='100%' border='0' cellpadding='0' cellspacing='0'><tr>" +
					"<td style='vertical-align: middle;'>" + (bc ? bc : data[col]) + "</td><td style='text-align: right;'><a class='nmdrTB_rowcontrol'>" +
	...
	$.resizeColumn = function (ev, mode, col) {
		if (mode == 0) {
			this.props.pageX = ev.pageX; 
			this.props.curCol = document.getElementById(this.id + "_nmdrTB_header_th" + col);
			this.props.curColWidth = this.props.curCol.offsetWidth;
		} else if (mode == 3 && this.props.curCol) {
			var diffX = ev.pageX - this.props.pageX;
			this.props.curCol.style.width = (this.props.curColWidth + diffX)+'px';
		} else if (mode == 4) {
			this.props.pageX = undefined;
			this.props.curCol = undefined;
			this.props.curColWidth = undefined
		}
	};
    ...
    $.openHeaderMenu = function (ev, src) {
    ...
    $.closeHeaderMenu = function (code, self, callback) {
        ...
        nmdr.core.animate.fadeOut(null, m, null, true, function () {
            ...
            if (code) self.handleCommand(null, code);
    ...
    $.openRowMenu = function (ev, row, cell) {
        ...
        nmdr.core.popup.open(m, s, 
			...
			function (callback) { self.closeRowMenu("clear", null, self, callback); }
		);
    ...
    $.initEvents = function () {
		...
		if (this.props.columnsResizable) {
			document.addEventListener('mousemove', function (e) { self.resizeColumn(e, 3); });
			document.addEventListener('mouseup', function (e) { self.resizeColumn(e, 4); });
		}
        ...
        document.addEventListener("keydown", function(ev) { 
            ...
            if (ev.keyCode == 40) {
                if (self.rowMenuOpen) {
                    if (self.rowMenuActRow != -1) {
                        self.updateRowMenuRow(false);
                        if (self.rowMenuActRow == self.menuCommands.length-1) self.rowMenuActRow = 0; 
						else self.rowMenuActRow++;
            ...
            // up arrow
                ...
                if (self.rowMenuOpen) {
                    if (self.rowMenuActRow != -1) {
                        self.updateRowMenuRow(false);
                        if (self.rowMenuActRow == 0) self.rowMenuActRow = self.menuCommands.length-1; 
						else self.rowMenuActRow--;
                    } else self.rowMenuActRow = 0;
                    self.updateRowMenuRow(true);
                }
            ...
            // enter key
                ...
                if (self.rowMenuOpen) {
                    ...
                    if (self.props.menuColumn != -1) {
						self.openRowMenu(ev, self.selectedRows[0]);
					}
                }
                nmdr.core.utils.stopPropagation(ev);
            }
    ...
    $.handleCommand = function (ev, name, row, leavOpen) {
		...
		if (ev) nmdr.core.utils.stopPropagation(ev);
		if (row) this.selectRows(ev, row);
        ...
        switch (name) {
            ...
            case "renameRow": break;
            case "copyContent": this.copyToClipboard(row); break;
            case "clear": break;
        }
    };

    $.executeMenuCommand = function (num) {
        this.closeRowMenu("clear", this.menuCommands[num]);
    }
    ...
    $.makeRowMenu = function (init, cell) {
		if (init) {
			return "<div class='nmdrTB_rowMenu' id='" + this.id + "_nmdrTB_rowMenu'></div>";
		}
    ...
    $.makeHeaderMenu = function () {
        ...
        buf.push("<tr>");
        buf.push("<td colspan='2'>");
        buf.push("<table border='0' width='100%'>");
        buf.push("<tr>");
        buf.push("<td><span id='" + this.id + "_nmdrTB_filterColumnName'>Column</span></td>");
        buf.push("<td><select id='" + this.id + "_nmdrTB_filterSelect'>" + this.prepareFilterPattern(false) + " style='width:100%'</select></td>");
		buf.push("<td style='width:100%;'><input id='" + this.id + "_nmdrTB_filterText' type='text' name='columnfilter' value='" + this.prepareFilterPattern(true) + "' style='width:100%;padding:0px;'/></td>");
		if (!this.props.showFilterData) {
			buf.push("<td style='width:16px;'><a href='' onClick=\"nmdr.core.$('" + this.id + "').handleCommand(event, 'setFilter');\">");
			buf.push("<img src='" + this.props.imagePath + "spFilterset.gif' style='border-style:none; vertical-align:middle;'/></a></td>");
			buf.push("</tr>");
		}
		else {
			buf.push("</tr>");
			buf.push("<tr><td>Selection</td>");
			buf.push("<td colspan='2' style='text-align:right'>");
			buf.push("<a href='' onClick=\"nmdr.core.$('" + this.id + "').handleCommand(event, 'uncheckAllFilter', null, true);\">");
			buf.push("<img src='" + this.props.imagePath + "spUncheckAll.gif' style='border-style:none; vertical-align:middle;'/></a>&nbsp;");
			buf.push("<a href='' onClick=\"nmdr.core.$('" + this.id + "').handleCommand(event, 'checkAllFilter', null, true);\">");
			buf.push("<img src='" + this.props.imagePath + "spCheckAll.gif' style='border-style:none; vertical-align:middle;'/></a>");
			buf.push("</td>");
			buf.push("</tr>");
			buf.push("<tr>");
			buf.push("<td colspan='3'>");
